...
#mscroll { position: fixed; z-index: 60; pointer-events: none; display: none;
  margin: -14px 0 0 -14px; }
body.mscrolling, body.mscrolling * { cursor: all-scroll !important;
...
// Press the middle button over anything scrollable: an anchor marker
// appears and moving the pointer pans, faster the further you move.
// Quick-click parks it (click any button to stop); press-drag-release
// scrolls only while held. Inputs are exempt so middle-click paste works.
(() => {
  const DEAD = 12;   // px of slack around the anchor before scrolling starts
  const SPEED = 8;   // px/second of scroll per px of pointer offset
  let sc = null;     // active scroll state, null when idle
  let swallow = false;  // eat the click/auxclick generated by ending a scroll

  const marker = el("div");
  marker.id = "mscroll";
  document.body.appendChild(marker);

  function arrows(canX, canY) {
    const a = ['<svg width="28" height="28" viewBox="0 0 28 28">',
      '<circle cx="14" cy="14" r="13" fill="rgba(23,23,29,.85)" stroke="#7aa2f7"/>'];
    if (canY) a.push('<path d="M14 4l3.2 4.5h-6.4z" fill="#e8e8ee"/>',
                     '<path d="M14 24l-3.2-4.5h6.4z" fill="#e8e8ee"/>');
    if (canX) a.push('<path d="M4 14l4.5-3.2v6.4z" fill="#e8e8ee"/>',
                     '<path d="M24 14l-4.5 3.2v-6.4z" fill="#e8e8ee"/>');
    a.push('<circle cx="14" cy="14" r="2" fill="#e8e8ee"/></svg>');
    return a.join("");
  }
  ...
  function scrollableFrom(node) {
    for (let n = node instanceof Element ? node : null; n; n = n.parentElement) {
      ...
      if (x || y) return { el: n, x, y };
    }
    const d = document.scrollingElement;
    const y = d.scrollHeight > d.clientHeight, x = d.scrollWidth > d.clientWidth;
    return x || y ? { el: d, x, y } : null;
  }

  function step(t) {
    if (!sc) return;
    const dt = sc.prev ? (t - sc.prev) / 1000 : 0;
    sc.prev = t;
    const v = (d) => Math.abs(d) > DEAD ? Math.sign(d) * (Math.abs(d) - DEAD) * SPEED * dt : 0;
    if (sc.canX) sc.el.scrollLeft += v(sc.px - sc.x);
    if (sc.canY) sc.el.scrollTop += v(sc.py - sc.y);
    sc.raf = requestAnimationFrame(step);
  }

  function start(e, hit) {
    sc = { el: hit.el, canX: hit.x, canY: hit.y, x: e.clientX, y: e.clientY,
           px: e.clientX, py: e.clientY, t0: performance.now(),
           moved: false, prev: 0, raf: 0 };
    marker.innerHTML = arrows(hit.x, hit.y);
    marker.style.left = e.clientX + "px";
    marker.style.top = e.clientY + "px";
    marker.style.display = "block";
    document.body.classList.add("mscrolling");
    sc.raf = requestAnimationFrame(step);
  }
  ...
  function stop() {
    ...
    document.body.classList.remove("mscrolling");
  }

  addEventListener("mousedown", (e) => {
    if (sc) {  // any button ends a parked scroll; swallow the resulting click
      e.preventDefault(); e.stopPropagation(); swallow = true; stop(); return;
    }
    if (e.button !== 1) return;
    const t = e.target instanceof Element ? e.target : null;
    if (t && t.closest("input, textarea, select, a")) return;
    const hit = scrollableFrom(t || document.body);
    if (!hit) return;
    e.preventDefault();
    swallow = true;
    start(e, hit);
  ...
  addEventListener("mousemove", (e) => {
    ...
    if (Math.hypot(e.clientX - sc.x, e.clientY - sc.y) > DEAD) sc.moved = true;
  }, true);

  addEventListener("mouseup", (e) => {
    if (!sc || e.button !== 1) return;
    if (sc.moved || performance.now() - sc.t0 > 300) stop();  // drag mode
    // else: parked — keeps scrolling until the next press/wheel/key
  }, true);

  for (const ev of ["click", "auxclick"]) {
    addEventListener(ev, (e) => {
      if (swallow || sc) { e.preventDefault(); e.stopPropagation(); }
      if (!sc) swallow = false;
    }, true);
  }
  addEventListener("wheel", stop, { passive: true, capture: true });
  addEventListener("keydown", () => { if (sc) stop(); }, true);
